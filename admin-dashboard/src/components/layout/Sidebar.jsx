...
export default function Sidebar() {
  ...
  return (
    <div 
      className={`flex h-screen flex-col justify-between border-r border-slate-800 bg-slate-950 p-4 text-slate-200 transition-all duration-300 ${
        isOpen ? 'w-64' : 'w-20'
      }`}
    >
      ...
      {/* User Logout Footer */}
      <div className="border-t border-slate-900 pt-4 px-1">
        <button 
          title={!isOpen ? "Log Out" : ""}
          className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-400 hover:bg-red-950/20 hover:text-red-400 transition-all overflow-hidden whitespace-nowrap ${
            !isOpen && 'justify-center'
          }`}
        >
